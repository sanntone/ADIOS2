...
: Engine("MhsReader", io, name, mode, std::move(comm))
{
  ...
  if(m_Mode== "daemon")
  {
    ...
    m_InquireIO->SetParameters({{"IPAddress", "10.0.0.61"}, {"Port", "9020"},{"TransportMode", "reliable"}});
    m_RemoteIO->SetParameters({{"IPAddress", "10.0.0.31"}, {"Port", "9010"},{"TransportMode", "reliable"}});
   ...
   while (true)
    {
        ...
        std::cout<<"********* daemon status"<<status<<" **********"<<std::endl;
...
//              varFloatsRemote.AddOperation("zfp", {{"accuracy", "0.01"}});
  ...
  else if(m_Mode == "inquirer")
  {
    ...
    std::cout<<"********* receive meta done *********"<<std::endl;
      ...
      //Engine *engine = &metaWriteIO->Open("Test", adios2::Mode::Read);
      ...
      for (const auto &variablePair : vars)
      {
          ...
          auto varMeta = metaWriteIO->InquireVariable<float>(variableName);
          if(varMeta){
              auto metaShape = varMeta->Shape();
              int metaDims =varMeta->Shape().size();
              std::vector<size_t> metaStart;
              metaStart.resize(metaDims);
              &m_IO.DefineVariable<float>(variableName,metaShape,metaStart,metaShape);
           }
    ...
    m_RemoteEngine = &m_RemoteIO->Open("stream", adios2::Mode::Read); //"Remote"
    m_InquireEngine = &m_InquireIO->Open("stream", adios2::Mode::Write); //"Inquire"
    m_SmartCacheEngine = &m_SmartCacheIO->Open(m_Name+"_cache", adios2::Mode::Write);
    //std::cout << "********** mhs Open done *********\n";
    ...
    std::cout << "**********mhs DefineVariable done *********\n";
  }
